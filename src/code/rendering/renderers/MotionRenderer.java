...
class MotionRenderer extends Renderer {
  ...
  public void initialise(Drawing d) {
    for (int y = 0; y < d.getHeight(); y++) {
      for (int x = 0; x < d.getWidth(); x++) {
        d.drawPixel(x, y, ((int)(Math.random()*2.0) * ~0) | -16777216);
      }
    }
  }
  ...
  public void render(Drawing d, Vector3 cameraPosition, Quaternion cameraRotation, RigidBody[] bodies) {
    double[] previousDepths = d.getDepths();
    ...
    Stream.of(bodies).parallel().forEach((b) -> {
      ...
      s.forEach((tri) -> assignTriDepths(d, tri));
    });
    ...
    IntStream.range(0, previousDepths.length).parallel().forEach((i) -> {
      if (previousDepths[i] == currentDepths[i]) return;
      int x = i%w;
      int y = i/w;
      if (
   ...
   */
  private void assignTriDepths(Drawing d, Tri3D tri) {
